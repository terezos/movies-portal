                ...
                <table class="table">
                    ...
                    @foreach ($moviePerCinema[$cinema->id] as $key => $movie)
                        <tr>
                            ...
                            <td style="display: flex"> <a href="{{ route('movie.edit', ['movie' => $movie]) }}" class="btn btn-success mx-1"><i class="fas fa-edit"></i></a>
                                <form method="POST" id="delete-form" action="{{route('movie.destroy', ['movie' => $movie])}}">
                                    @csrf
                                    @method('DELETE')
